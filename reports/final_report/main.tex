...
For the comparative study of canonical deep reinforcement learning methods, we selected the continuous-control benchmark \textbf{LunarLanderContinuous-v3}. This task is widely used to evaluate learning in a moderately high-dimensional continuous action space under stochastic initial conditions and non-linear contact dynamics.
...
We studied three algorithms spanning key design axes: \textbf{PPO} (on-policy, trust-region inspired), \textbf{A2C} (on-policy, actor--critic with $n$-step returns), and \textbf{SAC} (off-policy, maximum-entropy).
...
\begin{table}[H]
    \centering
    \caption{Benchmark setup on LunarLanderContinuous-v3 (fixed budget $100{,}000$ steps, seed $42$).}
    \begin{tabular}{lccc}
        \toprule
...
\begin{figure}[H]
    ...
    \caption{Smoothed episodic return (rolling mean over $50$ episodes) for PPO, A2C, and SAC on LunarLanderContinuous-v3.}
\end{figure}

\subsection*{Quantitative Summary}
For a representative run at seed $42$ and a fixed budget of $100{,}000$ steps, the last-$50$-episode statistics were:
\begin{itemize}
    \item PPO: mean return $\approx 86.27$ with standard deviation $\approx 65.37$.
    \item SAC: mean return $\approx 83.40$ with standard deviation $\approx 168.88$.
    \item A2C: mean return $\approx -188.51$ with standard deviation $\approx 43.15$.
\end{itemize}
...
\begin{itemize}
    ...
    \item \textbf{Sensitivity and reproducibility limitations:} The benchmark script trains a single run per method with a fixed seed. Consequently, claims about variance across seeds cannot be made decisively. Observed variance should therefore be interpreted as \emph{within-run} stochasticity rather than a full generalization assessment.
\end{itemize}

\newpage
\section*{Part 2: Custom Environment Design \& Training}
...
\[
x_b\sim \mathrm{Unif}(400,700),\quad v_b\sim \mathrm{Unif}(-10,-5),\quad w\sim \mathrm{Unif}(-3,3),\quad A\sim \mathrm{Unif}(20,50),\quad f\sim \mathrm{Unif}(0.5,2.0).
\]
...
\subsection{Transition Dynamics $T$}
...
\[
\alpha=10^{-4},\quad n_{\mathrm{steps}}=2048,\quad \text{batch size}=64,\quad \lambda_{\mathrm{GAE}}=0.95,\quad \epsilon=0.2,\quad c_e=0,\quad c_v=0.5,\quad \|\nabla\|_{\max}=0.5.
\]
...
\begin{itemize}
    ...
    \item \textbf{Statistical reporting:} The benchmark comparison was performed at a fixed seed; additional multi-seed runs are needed for statistically robust conclusions.
\end{itemize}
